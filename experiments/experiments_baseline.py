...
for dataset_name in datasets:
            ...
            if len(relevant_train_cases_bucket) == 0:
                ...
                if len(set(train_y_class)) < 2:
                    ...
                    for _, group in test_all_grouped:
                        test_y_class.extend(dataset_manager.get_class_label(group))
    ...
    with open(outfile, 'w') as fout:
        ...
        dt_results = pd.DataFrame({"actual": test_y_class, "predicted": pred_y_class, "nr_events": nr_events_all})
        dt_results.to_csv(f'truth_prediction/{dataset_name}_baseline.csv', index=False)
