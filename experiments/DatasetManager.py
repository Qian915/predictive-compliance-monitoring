...
class DatasetManager:
    ...
    def __init__(self, dataset_name):
        ...
        self.magnitude_col = dataset_confs.magnitude_col[self.dataset_name]
    ...
    def read_dataset(self):
        ...
        data = pd.read_csv(dataset_confs.filename[self.dataset_name], dtype=dtypes)
        return data
    ...
    def generate_prefix_data(self, data, min_length, max_length, gap=1):
        ...
        for nr_events in range(min_length+gap, max_length+1, gap):
            ...
            tmp[self.case_id_col] = tmp[self.case_id_col].apply(lambda x: "%s_%s"%(x, nr_events))   # modified case id with nr_events
    ...
    def get_regression_label(self, data):
        return data.groupby(self.case_id_col).first()[self.magnitude_col]
