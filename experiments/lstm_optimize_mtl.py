...
def objective(params):
    ...
    for train_index, val_index in kfold.split(train_x):
        ...
        class_weights = compute_class_weight(
            ...
            classes=np.unique(y_train_class_fold),
            y=y_train_class_fold
        )
        classification_sample_weights = np.array([class_weights[int(label)] for label in y_train_class_fold])
        regression_sample_weights = np.ones_like(y_train_values_scaled_fold)
        sample_weights = {
            'binary_output': classification_sample_weights,  # Weights for the classification task
            'regression_output': regression_sample_weights  # Uniform weights for regression
        }
        '''
        # Train the model
        history = model.fit(
...
dataset_ref_to_datasets = {
    ...
    "o2c": ["o2c"],
    "bpic2012w": ["bpic2012w_2"]    #TODO only optmize for bpic2012w_1 first! , "bpic2012w_2"
}
...
for dataset_name in datasets:
    ...
    space = {
        ...
        'dense_dropout': hp.uniform('dense_dropout', 0.1, 0.5),
    ...
    best = fmin(
        fn=objective,  # Use the logging wrapper function
