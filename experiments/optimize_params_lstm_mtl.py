...
def objective(params):
    ...
    for train_index, val_index in kfold.split(train_x):
        ...
        def create_multitask_model(input_shape, lstm_units, dropout_rate, dense_dropout):
            ...
            binary_output = Dense(1, activation='sigmoid', name='binary_output')(dense_out)
            # Output for regression task (linear for continuous values)
            regression_output = Dense(1, activation='linear', name='regression_output')(dense_out)
            # Define the model with the two outputs
            model = Model(inputs=input_layer, outputs=[binary_output, regression_output])
        ...
        model = create_multitask_model(input_shape, params['lstm_units'], params['dropout_rate'], params['dense_dropout']) #TODO define hyper params. for optimization here
        model.compile(optimizer=Adam(learning_rate=params['learning_rate']), 
                    loss={'binary_output': 'binary_crossentropy', 'regression_output': 'mean_squared_error'}, 
                    metrics={'binary_output': ['AUC'], 'regression_output': ['mae']})       #TODO modified metric representation
        early_stopping = EarlyStopping(monitor='val_loss', patience=3, restore_best_weights=True)       #TODO define early stoppping with patience of 3 to avoid over-fitting
        # Train the model with the current fold
        history = model.fit(
            x_train_fold, {'binary_output': y_train_class_fold, 'regression_output': y_train_values_scaled_fold},   # for classification and regression!
            validation_data=(x_val_fold, {'binary_output': y_val_class_fold, 'regression_output': y_val_values_scaled_fold}),
            epochs=5,      #TODO reduce epochs for faster training!
            batch_size=params['batch_size'],  #TODO best batch size after hyper-params optimization
            verbose=0,
            callbacks=[early_stopping]      #TODO add early stopping
        )
        #print("Available metrics in history:", history.history.keys())
        # Get validation AUC and MAE for this fold (access the correct key)
        val_auc = history.history['val_binary_output_AUC'][-1]  #TODO AUC value from the validation set
        auc_scores.append(val_auc)
        val_mae = history.history['val_regression_output_mae'][-1]  #TODO MAE
        mae_scores.append(val_mae)
        #val_loss = history.history['val_loss'][-1]
        #val_losses.append(val_loss)
    
    # Average AUC and MAE over the 3 folds and combine them
    mean_auc = np.mean(auc_scores)
    mean_mae = np.mean(mae_scores)
    alpha = 0.5  # Weight for AUC
    beta = 0.5  # Weight for MAE
    total_loss = -alpha * mean_auc + beta * mean_mae
